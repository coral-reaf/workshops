...
- [ワークショップ演習 - アップグレード前レポートのレビュー](#ワークショップ演習---アップグレード前レポートのレビュー)
  ...
  - [ガイド](#ガイド)
    ...
    - [チャレンジラボ: 多数のハイリスクの検出結果を無視できるのはどうでしょうか?](#チャレンジラボ-多数のハイリスクの検出結果を無視できるのはどうでしょうか)
  - [まとめ](#まとめ)
...
- RHEL8 ペットアプリサーバーの 1 つを参照していることを確認したら、メインメニューから ツール > アップグレードレポート に移動します。これにより、選択したホスト用に生成された Leapp アップグレード前レポートが表示されます。たとえば、レポートは次のようになります。`No data to show....`で何も表示されない場合は、ブラウザの表示を更新すれば表示されます。:
...
### チャレンジラボ: 多数のハイリスクの検出結果を無視できるのはどうでしょうか?
...
- もちろん、リスクを軽減するたのベストプラクティスはたくさんあります。最初に開発環境やテスト環境でアップグレードを試して、アプリケーションへの影響をテストすること。開発サーバーとテスト サーバーで問題が解決されていれば、運用環境でそれらの問題の発生を防ぐことが可能です。
