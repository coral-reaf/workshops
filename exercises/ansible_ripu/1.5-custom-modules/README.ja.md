...
- Leapp カスタム アクターの開発は、Playbook にタスクを追加する場合比べると簡単ではありません。そのため、ほとんどのカスタムアクター開発は、Ansible を使用して実現するのが最適です。タスクは、RHEL OS アップグレードを実際に実行するためにアップグレード Playbook の中の `infra.leapp` コレクション `upgrade` ロールをインポートするタスクの前後に含めるとよいです。
